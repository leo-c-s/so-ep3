...
FileMeta *FileMeta::read_meta(std::fstream &infile, int *address)
{
    ...
    { // if type is invalid or zero, the file does not exist
        return nullptr;
    }
}
...
void Directory::del_file(int index)
{
    if (index < this->get_file_count())
    {
...
Filesystem::Filesystem(std::string filesystem_path)
{
    ...
    else
    {
        ...
        // create FAT
        for (int i = 0; i < 5; i++)
        { // make linked list for root directory
...
void Filesystem::rm(std::string file_path)
{
    ...
    for (int j = 0; j < cur->get_file_count(); j++)
    {
